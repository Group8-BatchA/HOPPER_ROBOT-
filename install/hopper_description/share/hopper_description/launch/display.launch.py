...
def generate_launch_description():
    pkg_share = launch_ros.substitutions.FindPackageShare(package='hopper_description').find('hopper_description')
    default_model_path = os.path.join(pkg_share, 'urdf/hopper.urdf.xacro')
    default_rviz_config_path = os.path.join(pkg_share, 'rviz/sensors.rviz')
    world_path=os.path.join(pkg_share, 'worlds/hopper_world.world'),
    ...
    arguments=['-entity', 'hopper_robot', '-topic', 'robot_description'],
    parameters= [{'use_sim_time': use_sim_time}],
    output='screen'
    )
